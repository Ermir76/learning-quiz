import React from 'react';

const ResultsPage = ({ setPage, results, quiz }) => {
    // Check if this is flashcard results or quiz results
    const isFlashcard = results.type === 'flashcard';
    
    // Calculate score and total based on result type
    const score = isFlashcard ? results.knownCount : results.filter(r => r.isCorrect).length;
    const total = isFlashcard ? results.totalCards : quiz.questions.length;
    const percentage = isFlashcard ? results.percentage : Math.round((score / total) * 100);

    return (
        <div className="w-full max-w-2xl mx-auto">
            <div className="bg-slate-800 p-8 rounded-xl shadow-2xl text-center">
                <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold text-white">
            📊 {results[0]?.type === 'flashcard' ? 'Flashcard' : 'Quiz'} Results
          </h1>
          <button
            onClick={() => setPage('categoryQuizList')}
            className="bg-slate-700 hover:bg-slate-600 text-white px-6 py-3 rounded-lg transition-colors"
          >
            ← Back to Study Sets
          </button>
        </div>
                <h2 className="text-4xl font-bold mb-2 text-slate-200">
                    {isFlashcard ? 'Flashcard Session Complete!' : 'Quiz Complete!'}
                </h2>
                <p className="text-xl text-slate-300 mb-6">
                    {isFlashcard ? 'Your Knowledge Score' : 'Your Score'}
                </p>
                <p className="text-6xl font-bold mb-2 text-slate-100">{score} / {total}</p>
                <p className="text-2xl font-semibold mb-4 text-slate-300">({percentage}%)</p>
                {quiz.modelUsed && (
                    <p className="text-xs text-slate-500 mb-8">Generated by: {quiz.modelUsed}</p>
                )}
                
                <div className="text-left my-8">
                    <h3 className="text-xl font-bold mb-4 text-slate-200">
                        {isFlashcard ? 'Review Your Cards' : 'Review Your Answers'}
                    </h3>
                    <div className="space-y-4 max-h-72 overflow-y-auto pr-4">
                        {isFlashcard ? (
                            // Flashcard results display
                            results.cards.map((card, index) => (
                                <div key={index} className={`p-4 rounded-lg ${card.known ? 'bg-green-900/50' : 'bg-red-900/50'}`}>
                                    <p className="font-semibold text-slate-200 mb-2">{index + 1}. {card.questionText}</p>
                                    <div className="text-sm space-y-1">
                                        <p className="text-slate-300">Answer: {card.answer}</p>
                                        <p className={`text-sm font-semibold ${card.known ? 'text-green-400' : 'text-red-400'}`}>
                                            {card.known ? '✓ You knew this' : '✗ You didn\'t know this'}
                                        </p>
                                    </div>
                                </div>
                            ))
                        ) : (
                            // Original quiz results display
                            results.map((result, index) => (
                                <div key={index} className={`p-4 rounded-lg ${result.isCorrect ? 'bg-green-900/50' : 'bg-red-900/50'}`}>
                                    <p className="font-semibold text-slate-200 mb-2">{index + 1}. {result.questionText}</p>
                                    {result.type === 'code-input' ? (
                                        <div className="text-sm space-y-2">
                                            <div className="text-slate-300">Your Answer:
                                                <pre className="bg-slate-900 p-2 rounded mt-1 overflow-x-auto"><code className="font-mono text-white">{result.selected}</code></pre>
                                            </div>
                                            {!result.isCorrect && (
                                                 <div className="text-slate-300">Correct Answer:
                                                    <pre className="bg-slate-900 p-2 rounded mt-1 overflow-x-auto"><code className="font-mono text-white">{result.correct}</code></pre>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="text-sm space-y-1">
                                            <p className="text-slate-300">Your answer: {result.selected}</p>
                                            {!result.isCorrect && <p className="text-slate-300">Correct answer: {result.correct}</p>}
                                            {result.explanation && (
                                                <div className="mt-2 pt-2 border-t border-slate-700">
                                                    <p className="text-xs text-slate-400">{result.explanation}</p>
                                                </div>
                                            )}
                                        </div>
                                    )}
                                    <a href={result.learnMoreUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-400 hover:underline mt-2 inline-block">Learn More</a>
                                </div>
                            ))
                        )}
                    </div>
                </div>

                <button 
                    onClick={() => setPage('categoryQuizList')}
                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-lg text-lg transition"
                >
                    Back to Quiz List
                </button>
            </div>
        </div>
    );
};

export default ResultsPage;