...
import { LibraryIcon, PencilIcon } from './Icons';

const OptionsPage = ({ setPage }) => (
  <div className="app-shell min-h-screen px-6 py-14">
    <div className="max-w-5xl mx-auto space-y-12 animate-fade-in">
      <div className="text-center space-y-4">
  <h1 className="heading-display flex items-center justify-center gap-4 text-5xl md:text-6xl text-slate-900 dark:text-slate-100 tracking-tight"><LibraryIcon className="w-14 h-14 text-accent" /> Study Hub</h1>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <button onClick={() => setPage('createQuiz')} className="card group relative overflow-hidden p-10 text-left">
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-500 bg-gradient-to-br from-accent/10 via-transparent to-transparent" />
            <div className="flex items-start justify-between mb-6">
              <div className="h-12 w-12 rounded-xl bg-accent text-white flex items-center justify-center shadow-glow"><PencilIcon className="w-7 h-7" /></div>
        ...
        <button onClick={() => setPage('quizCategories')} className="card group relative overflow-hidden p-10 text-left">
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-500 bg-gradient-to-br from-accent/10 via-transparent to-transparent" />
            <div className="flex items-start justify-between mb-6">
              <div className="h-12 w-12 rounded-xl bg-accent text-white flex items-center justify-center shadow-glow"><LibraryIcon className="w-7 h-7" /></div>
